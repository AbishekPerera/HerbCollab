...
router.route("/signup").post([], async (req, res, next) => {
  ...
  if (existingUserName) {
    return res
      .status(406)
      .json({ message: "UserName already exists! Use another UserName" });
...
router.route("/update/:id").put(async (req, res) => {
  ...
  await users
    ...
    .catch((err) => {
      console.log(err);
      res.status(400).json({ status: "Error with update user" });
    });
});
...
router.route("/updateStatus/:id").put(async (req, res) => {
  ...
  await users
    ...
    .catch((err) => {
      console.log(err);
      res.status(400).json({ status: "Error with update status" });
    });
});
...
router.route("/updatePassword/").put(async (req, res) => {
  ...
  await users
    .findByIdAndUpdate(result._id, changePassword, {
      new: true,
    })
    .then(() => {
